 ...
 * root; call `const ok = await confirm('Delete this?')` from any handler.
      ...
      {state && (
        <div className="fixed inset-0 z-[100] flex items-center justify-center p-4">
          ...
          <div className="relative bg-white rounded-2xl shadow-floating border border-ui-line max-w-sm w-full p-5 sm:p-6">
            ...
            <p className="text-sm text-ui-muted leading-relaxed">{state.message}</p>
            <div className="flex gap-2.5 mt-4">
              ...
              <button
                onClick={() => settle(true)}
                className={`flex-1 inline-flex items-center justify-center gap-1.5 rounded-xl px-4 py-2.5 sm:py-2 text-sm font-medium transition-all active:scale-[0.98] ${
