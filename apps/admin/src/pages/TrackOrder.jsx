...
export default function TrackOrder() {
  ...
  useEffect(() => {
    trackOrder(trackingId)
      .then(setOrder)
      .catch(() => setError('We could not find an order with this tracking ID.'));
  }, [trackingId]);
    ...
    <div className="min-h-screen flex items-start justify-center px-4 sm:px-5 py-10 sm:py-16">
      <div className="w-full max-w-lg">
        <div className="text-center mb-8">
          <div className="text-xs uppercase tracking-[0.2em] text-ui-muted">Track your order</div>
          ...
          <div className="bg-ui-panel border border-ui-line rounded-xl shadow-card p-6">
            ...
            {order.courier?.trackingCode && (
              <div className="mb-4 text-xs font-mono text-ui-muted">
                Courier tracking code: <span className="text-ui-ink">{order.courier.trackingCode}</span>
              </div>
            )}

            <h3 className="text-xs uppercase tracking-wide text-ui-muted mb-3">Where's the parcel now</h3>
                    ...
                    {group.entries.map((entry, idx) => (
                      <li key={idx} className="flex gap-3">
                        ...
                        <span className="flex-1 min-w-0">
                          <span className="flex items-center gap-1.5 text-sm font-medium text-ui-ink capitalize">
                            {entry.kind === 'status' ? (
            ...
            <div className="border-t border-dashed border-ui-line pt-4">
              <h3 className="text-xs uppercase tracking-wide text-ui-muted mb-2">Items</h3>
