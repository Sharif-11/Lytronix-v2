...
export default function UpdatePrompt() {
  ...
  return (
    <div className="fixed inset-x-3 bottom-3 z-[60] sm:inset-x-auto sm:right-4 sm:w-80">
      <div className="flex items-center gap-2.5 rounded-xl bg-ui-ink text-white shadow-floating px-4 py-3">
        <RefreshCw size={16} className="shrink-0" />
        <span className="text-sm flex-1">A new version is available.</span>
        <button
          onClick={() => window.location.reload()}
          className="text-sm font-semibold bg-white/15 hover:bg-white/25 rounded-lg px-2.5 py-1"
        >
          Reload
        </button>
        <button
