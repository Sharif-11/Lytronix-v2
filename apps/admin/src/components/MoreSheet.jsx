...
import { BookUser, Users, ShieldCheck, LogOut, X, Wallet, Landmark, Smartphone, FolderTree, TrendingUp, UserCircle, Megaphone, Inbox, MessagesSquare } from 'lucide-react';
    ...
    <div className="sm:hidden fixed inset-0 z-40" role="dialog" aria-modal="true">
      ...
      <div className="absolute bottom-0 inset-x-0 bg-white rounded-t-3xl border-t border-ui-line pb-[env(safe-area-inset-bottom)] max-h-[80vh] overflow-y-auto shadow-floating">
        ...
        <div className="px-5 pb-6 space-y-2">
          ...
          {hasPermission('payments:manage') && (
            <NavLink to="/sms-listener" onClick={onClose} className={itemClass}>
              <Smartphone size={18} /> {t('nav.smsListener')}
            </NavLink>
          )}
          {hasPermission('superadmin') && (
            <NavLink to="/bank-details" onClick={onClose} className={itemClass}>
              <Landmark size={18} /> {t('nav.bankDetails')}
            </NavLink>
          )}
