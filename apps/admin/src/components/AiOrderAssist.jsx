...
// Paste customer text (or, when the active provider supports it, a
// screenshot — a chat message, a Steadfast label, an order form photo) ->
// the backend asks the configured AI provider (Gemini free tier, or
// Anthropic) to pull out the order details, matching the district/thana onto
// the Steadfast hub list -> the admin reviews the draft and clicks "Fill the
...
export default function AiOrderAssist({ onApply }) {
  ...
  useEffect(() => {
    getAiConfig()
      .then(setCfg)
      .catch(() => setCfg({ configured: true, imageSupported: false })); // don't block the panel on a meta-endpoint hiccup
  }, []);
        ...
        <div className="px-4 pb-4 space-y-3">
          {!cfg.configured && (
            <p className="text-xs text-ui-rust bg-red-50 border border-red-100 rounded-lg px-2.5 py-2">
              AI assist isn't configured yet. Add a free <code>GEMINI_API_KEY</code> (get one at{' '}
          ...
          <div className="flex items-center gap-3">
            ...
            <button
              type="button"
              onClick={extract}
              disabled={loading || !cfg.configured}
