...
const previewOf = (m) =>
  ...
  m.type === 'video' ? '🎥 ভিডিও' :
  m.type === 'album' ? `📷 ${m.media?.length || 0}টি ফাইল` :
  (m.body || '').slice(0, 120);
...
export default function ChatWidget({ hint = '' }) {
  ...
  const sendPendingDrops = async () => {
    ...
    setDropUploadPct(0);
    // Thumbnails intentionally stay visible (pendingDrops isn't cleared
    // yet) for the whole upload — only cleared below once the message is
    // actually sent. On failure they're left in place so the customer can
    // just hit Send again without re-picking the files.
    ...
    try {
      ...
      setError(err.response?.data?.message || 'ফাইল পাঠানো যায়নি।'); // pendingDrops left as-is so Send can be retried
    } finally {
              ...
              {pendingDrops.length > 0 || sendingDrops ? (
                <div className="shrink-0 bg-[#F0F0F0] px-3 py-2.5 space-y-2.5">
                  <div className="flex items-center justify-between">
                    ...
                    <button type="button" onClick={cancelPendingDrops} disabled={sendingDrops} className="text-xs text-ui-rust hover:underline font-bangla disabled:opacity-40">
                      বাতিল করুন
                    </button>
                  </div>
                    ...
                    {pendingDrops.map((p) => (
                      <div key={p.id} className="relative w-16 h-16 shrink-0 rounded-xl overflow-hidden border border-black/10 bg-white">
                        ...
                        <button
                          ...
                          className="absolute -top-1.5 -right-1.5 w-5 h-5 rounded-full bg-white border border-black/10 text-ui-rust flex items-center justify-center shadow-sm disabled:opacity-40"
                          aria-label="Remove"
                        >
                          <X size={11} />
                        </button>
